...
export function AccountForm() {
  ...
  const { data: languageData, isLoading: isLanguageLoading } = useQuery({
    queryKey: ['languageSetting'],
    queryFn: async () => {
      const response = await axios.get('/api/users/get-language');
  ...
  const updateProfileMutation = useMutation({
    mutationFn: async (data: AccountFormValues) => {
      ...
      return axios.post('/api/users/update-profile', formData);
    },
    onSuccess: async (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['languageSetting'] });
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <Button
          type="submit"
          disabled={updateProfileMutation.isPending || isLanguageLoading}
        >
